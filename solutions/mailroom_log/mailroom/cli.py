    ...
    def send_thank_you(self):
        ...
        while True:
            ...
            if name == "list":
                ...
                logging.info('have a donor name: %s', name)
                break
        ...
        while True:
            ...
            try:
                ...
                if math.isnan(amount) or math.isinf(amount) or round(amount, 2) == 0.00:
                    logging.error('we caught this one using math :P')
                    raise ValueError
            # in this case, the ValueError could be raised by the float() call,
            # or by the NaN-check
            except ValueError:
                logging.error('amount %s was not valid, caught ValueError', amount_str)
        ...
        if donor is None:
            logging.info("adding new donor: %s", name)
            donor = self.db.add_donor(name)

        # Record the donation
        donor.add_donation(amount)
        logging.info("added donation of $%.2f for %s", amount, name)
